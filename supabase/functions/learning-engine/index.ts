...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🧠 Learning Engine: Analyzing trade history for all users...');
    ...
    if (!allTrades || allTrades.length === 0) {
      console.log('No trade data found for analysis');
    ...
    const tradesByUser = new Map<string, any[]>();
    ...
    console.log(`📊 Found ${tradesByUser.size} users with trades`);
    ...
    for (const [userId, trades] of tradesByUser.entries()) {
      if (trades.length < 10) {
        console.log(`⚠️ User ${userId}: Not enough trades (${trades.length}/10 minimum)`);
        continue;
      }

      console.log(`🔄 Processing user ${userId}: ${trades.length} trades`);

      // Calculate comprehensive statistics for this user
      ...
      console.log(`📊 User ${userId} - WinRate: ${stats.winRate.toFixed(2)}%, Expectancy: ${stats.expectancy.toFixed(2)}`);
      ...
      if (upsertError) {
        console.error(`❌ Failed to save analysis params for user ${userId}:`, upsertError);
      } else {
        console.log(`✅ Analysis parameters updated for user ${userId}`);
        results.push({
          userId,
          stats,
          insights,
